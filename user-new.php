...
foreach ( array( 'user_login' => 'login', 'first_name' => 'firstname', 'last_name' => 'lastname',
				'email' => 'email', 'sms_sender_contact' => 'smssendercontact', 'url' => 'uri', 'role' => 'role', 'send_password' => 'send_password', 'noconfirmation' => 'ignore_pass' ) as $post_field => $var ) {
	$var = "new_user_$var";
...
<table class="form-table">
	...
	<tr class="form-field form-required">
		<th scope="row"><label for="sms_sender_contact"><?php _e('Phone Number'); ?> <span class="description"><?php _e('(required)'); ?></span></label></th>
		<td><input name="sms_sender_contact" type="text" id="sms_sender_contact" value="<?php echo esc_attr($new_user_smssendercontact); ?>" /></td>
	</tr>
<?php if ( !is_multisite() ) { ?>
	<tr class="form-field">
